...
$('#nav-current').text(search)
var searchsect= new Vue({
...
methods: {
   fetch: function(num){
    $('#prod-loader').show()
  const pagenum=num
  const url = `${API_URL}/api/search/${search}/${pagenum}`; 
...
$('.orderby').on('change', function() {
    if(this.value=='price-asc'){
      urlprod=`${API_URL}/api/searchproductspriceasc`
      searchsect.fetch(1)
      searchsect.$forceUpdate();
    }else if(this.value=='menu_order'){
        urlprod=`${API_URL}/api/search`
        searchsect.fetch(1)
    }else if(this.value=='price-desc'){
        urlprod=`${API_URL}/api/searchproductspricedesc`
        searchsect.fetch(1) 
    }else if(this.value=='date'){
        urlprod=`${API_URL}/api/searchbydate`
        searchsect.fetch(1)  
    }else if(this.value=='rating'){
        urlprod=`${API_URL}/api/searchbyrating`
        searchsect.fetch(1)  
    }else if(this.value=='popularity'){
        urlprod=`${API_URL}/api/searchbypopularity`
        searchsect.fetch(1)  
    }
  });
